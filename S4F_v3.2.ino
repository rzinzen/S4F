...
Version 3.1
  MINOR:
    - introduced if-loop to make sure temp measurements are not skewed due to bad contactVersion 3.1
Version 3.2
  MINOR:
    - introduced timer in display and re-aranged display info
*/
...
void loop() {
  TimeNow = millis();
  IntervalInfo = 1 * Interval;    //sets measurement interval for information report (in milliseconds)
...
////Report to LCD
  ...
  //Time Report
    MinSinceStart = int(((TimeNow - TimeStart)/1000)/60);
    SecSinceStart = int(((TimeNow - TimeStart)/1000) - (MinSinceStart * 60));
    if (MinSinceStart<10) {
      lcd.setCursor(12,0);
    } else {
      lcd.setCursor(11,0);
    }
    lcd.print(MinSinceStart);
    lcd.print(":");
    if (SecSinceStart<10) {
      lcd.print("0");
    }
    lcd.print(SecSinceStart);
