...
//Time values  (Note: if stored as unsigned 4-byte long values, overflow limit is ~50 days)
  unsigned int Std = 1500;            //sets a standard variable for time delays 
  ...
  unsigned long Duration;             //sets a variable to store a time value (here: Current time) as a long (4bytes) unsigned integer
...
//Thermistor variables
  ...
  float Temp_C = 0.0;               //defines the variables for (for temperature) as a decimal (Temp in Kelvin, in Celsius, and average temp in Celsius)
  float Temp_K, TempAvg;    //defines the variables for (for temperature) as a decimal (Temp in Kelvin, in Celsius, and average temp in Celsius)
...
//Hall Effect Sensor variables
  ...
  //CORRECT XXX CORRECT THIS CPL VALUE !!! XXX
  ...
  if (digitalRead(ButtonPin) == HIGH) {
    ...
    while (ButtonState == 0 ) {
      //LCD - Duration Info
        ...
        lcd.setCursor(1, 1);
        lcd.print(float(Duration)/(1000.0*60.0));
        lcd.print(" min");
        delay(Std*2);
      //LCD - Volume Info
        lcd.clear();
...
void MeasureFlow() {
  ...
  FlowRate = CPM / CPL;                           //calculates current FlowRate
  N_FlowRate++;
  FlowRateSum = FlowRateSum + FlowRate;   //keeps a running sum of flow rate measurements
  FlowRateAvg = FlowRateSum / N_FlowRate;   //updates the average temperature measured since start
  HallCount = 0;   //updates the average temperature measured since start
